...
int main(int argc, char* argv[])
{
	...
	/*ローカルアドレスへのバインド*/
	bind_return = bind(servSock, (struct sockaddr*)&echoServAddr, sizeof(echoServAddr));

	if (bind_return < 0)
		DieWithError("bind() failed", WSAGetLastError());

	fprintf(stdout, "bind : %s", WSAGetLastError());

	/*「接続要求をリスン中」というマークをソケットにつける*/
	if (listen(servSock, MAXPENDING) < 0)
	{
		DieWithError("listen() failde", WSAGetLastError());
	}
	...
	for (;;)
	{
		...
		{
			DieWithError("accepr() failed", WSAGetLastError());
		}
